...
class HormonalMetacontroller:
    ...
    async def process_hormonal_response(self, agent_id: str, xi: float) -> Dict[str, Any]:
        ...
        # 1.2 Busca o Mapa Endócrino (quais vetores usar?)
        endocrine_map = await self.state_manager.get_endocrine_map(agent_id)

        # 2. Calculate Response
        steering_result = self._calculate_dosage(xi, drives, current_hormones, endocrine_map)
    ...
    def _calculate_dosage(self, xi: float, drives: MotivationalDrives,
                          hormones: Dict[str, float],
                          endocrine_map: Dict[str, str]) -> Dict[str, Any]:
        ...
        # 1. Resposta de Estresse (CORTISOL)
        if effective_stress > 0.6:
            result["state_label"] = "STRESSED"
            result["hormonal_injection"] = "[STEERING: CAUTION - DEFENSIVE]"

            # Pega o vetor que estiver associado ao Cortisol no momento
            vector_name = endocrine_map.get("cortisol", "Stoic_Calmness")

            result["active_steering"] = {
                "concept": vector_name,
                "intensity": 5.0,  # Alta intensidade para defesa
                "layer_idx": 18
            }

        # 2. Resposta de Conexão (OXITOCINA)
        elif drives.connection.intensity > 0.7:
            result["state_label"] = "EMPATHIC_FLOW"
            result["hormonal_injection"] = "[STEERING: EMPATHY - WARM]"

            vector_name = endocrine_map.get("oxytocin", "High_Empathy")
        ...
        # 3. Resposta de Curiosidade (DOPAMINA)
        elif drives.curiosity.intensity > 0.7:
            result["state_label"] = "FLOW"
            result["hormonal_injection"] = "[STEERING: CURIOSITY - EXPLORATIVE]"

            vector_name = endocrine_map.get("dopamine", "Socratic_Questioning")
