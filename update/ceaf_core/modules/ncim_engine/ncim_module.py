...
class NCIMModule:
    ...
    **INSTRUÇÕES:**
    ...
    async def get_current_self_model(self) -> CeafSelfRepresentation:
        """
        Recupera o auto-modelo atual do MBS.
        Se não existir, retorna um padrão.
        """
        try:
            # Busca a memória singleton de identidade
            mem = await self.memory.get_memory_by_id(SELF_MODEL_MEMORY_ID)

            # Verifica se é válida e reconstrói o objeto Pydantic
            if mem and hasattr(mem, 'content') and mem.content.structured_data:
                return CeafSelfRepresentation(**mem.content.structured_data)

        except Exception as e:
            logger.error(f"NCIM: Erro ao recuperar self-model do MBS: {e}")

        # Fallback: Retorna um modelo novo ("nascido hoje") se falhar
        logger.warning("NCIM: Self-model não encontrado ou inválido. Retornando padrão.")
