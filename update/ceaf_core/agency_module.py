...
class AgencyModule:
    ...
    def __init__(self, llm_service: LLMService, vre_engine: VREEngineV3, mcl_engine: 'MCLEngine',
        ...
        self.physics_model = StatePredictor()  # Forward Model
        self.inverse_model = ActionGenerator()  # Inverse Model
        try:
            self.physics_model.load_state_dict(torch.load("./aura_brain/forward.pth"))
            self.physics_model.eval()  # Modo de leitura
            logger.info("✅ AgencyModule: Córtex Geométrico carregado.")
        except:
            logger.warning("⚠️ AgencyModule: Cérebro não treinado. Rodando em modo puramente LLM.")
    ...
    async def decide_next_step(self, cognitive_state: CognitiveStatePacket, mcl_guidance: Dict[str, Any],
        ...
        # [NOVO] Dicionário para rastrear penalidades acumuladas por feedback do VRE
        ...
        for step in range(MAX_RECURSIVE_STEPS):
            ...
            for candidate in all_candidates:
                ...
                # [HOTFIX] Aplicar penalidade se este candidato já tiver sido marcado anteriormente
                penalty = candidate_penalties.get(candidate.candidate_id, 0.0)
                if penalty > 0:
                    logger.info(f"Aplicando penalidade de {penalty:.2f} ao candidato '{candidate.candidate_id}'")
                    value -= penalty
            ...
            # [HOTFIX] Em vez de substituir, penalizamos o atual, geramos novos e somamos tudo

            # 1. Penaliza o candidato atual para que ele não ganhe automaticamente na próxima, a menos que seja muito superior
            current_penalty = candidate_penalties.get(best_candidate.candidate_id, 0.0)
            candidate_penalties[
                best_candidate.candidate_id] = current_penalty + 0.15  # Penalidade cumulativa (ajuste conforme necessário)

            # 2. Gera novos candidatos com base no feedback
            new_candidates = await self._generate_action_candidates(
                cognitive_state, mcl_guidance, observer,
                chat_history, limit=config["max_candidates"],
                known_capabilities=known_capabilities
            )

            # 3. Mantém os antigos E adiciona os novos (Extend em vez de Overwrite)
            all_candidates.extend(new_candidates)

            logger.info(
                f"HOTFIX: {len(new_candidates)} novos candidatos adicionados. Total agora: {len(all_candidates)}. Candidato '{best_candidate.candidate_id[:8]}' penalizado.")
        ...
        for candidate in all_candidates:
            ...
            # Aplicar a penalidade novamente na decisão final
            penalty = candidate_penalties.get(candidate.candidate_id, 0.0)
            if penalty > 0:
                logger.info(f"Mantendo penalidade de {penalty:.2f} para '{candidate.candidate_id}' na decisão final.")
                value -= penalty
