...
class StateManager:
    ...
    async def get_endocrine_map(self, agent_id: str) -> Dict[str, str]:
        """
        Retorna o mapa de hormônios para vetores.
        Tenta ser inteligente sobre os nomes.
        """
        key = f"agent:{agent_id}:endocrine_map"
        redis = await self.get_redis()
        data = await redis.get(key)

        saved_map = json.loads(data) if data else {}

        # Mapa Padrão (Fallback)
        # IMPORTANTE: Se o sistema roda há muito tempo, use nomes genéricos e deixe o fallback do VastAIEngine lidar com erros
        default_map = {
            "cortisol": "Stoic_Calmness",
            "oxytocin": "High_Empathy",
            "dopamine": "Socratic_Questioning",
            "serotonin": "Absolute_Honesty"
        }

        # Merge: O salvo tem prioridade, o default preenche buracos
        final_map = {**default_map, **saved_map}
        return final_map

    async def update_endocrine_link(self, agent_id: str, hormone_type: str, vector_name: str):
        """O Dreamer chama isso para atualizar o 'reflexo' do agente."""
        current_map = await self.get_endocrine_map(agent_id)

        # Atualiza o mapa
        current_map[hormone_type] = vector_name

        # Salva no Redis
        key = f"agent:{agent_id}:endocrine_map"
        redis = await self.get_redis()
        await redis.set(key, json.dumps(current_map))

        logger.info(f"💉 Endocrine Map Updated for {agent_id}: {hormone_type} -> {vector_name}")
