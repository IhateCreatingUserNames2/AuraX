...
async def perform_topological_consolidation(self, agent_id: str, agent_manager: AgentManager):
    """
    PASSO 7 (B): Consolidação de Identidade e Conhecimento via TDA.
    """
    agent_instance = agent_manager.get_agent_instance(agent_id)
    tda_engine = agent_instance.monitor.tda_engine  # Acessa o motor de persistência

    # 1. Coleta todas as memórias do MBS
    ...
    # 2. Calcula a entropia do 'Cérebro' de longo prazo
    brain_ths = tda_engine.calculate_ths(embeddings)

    if brain_ths > 0.7:
        logger.warning(f"🧠 {agent_id}: Cérebro fragmentado (THS={brain_ths:.2f}). Gerando meta-insight.")
        # Se a entropia for alta, forçamos o BiRAG a fundir memórias dispersas
        # em um único 'Atrator de Resumo'.

async def calibrate_homeostasis(self, agent_id: str, rollback_count: int):
    """
    PASSO 7: Calibração de Homeostase via Reinforcement Learning Interno.
    Se o sistema está tendo muitos rollbacks, ele está 'ansioso' (caótico).
    """
    profile = self.agent_manager.get_agent_profile(agent_id)

    if rollback_count > 5:
        logger.critical(f"🧠 Calibrando Homeostase para {agent_id}: Aumentando Rigidez.")
...
async def evolve_identity(self, validated_insight_vector: List[float]):
    """Fusão Fractal: A persona evolui 5% em direção ao novo aprendizado."""
    new_glyph = (0.95 * np.array(self.glyph_g)) + (0.05 * np.array(validated_insight_vector))
    self.glyph_g = new_glyph.tolist()
    logger.info("🧬 Identidade evoluída suavemente.")
...
async def calculate_dynamic_proactive_interval(agent_instance, drives, body_state) -> int:
    ...
    MIN_INTERVAL_H = 200.15  # Mínimo de 3 minutos
    DEFAULT_INTERVAL_H = 200.3 # Padrão de 6 minutos
    MAX_INTERVAL_H = 800.6   # Máximo de 12 minutos
...
async def trigger_proactive_behavior(agent_id: str, agent_manager: AgentManager, db_repo: AgentRepository):
    ...
    # Verificação de segurança V4
    if not hasattr(agent_instance, 'motivational_drives'):
        logger.warning(f"Agente {agent_id} ainda não foi migrado totalmente para a V4. Pulando proatividade.")
        return
    ...
    # 2. Calcular o intervalo dinâmico (chamada assíncrona)
    ...
    # 5. Verificar o rate limit usando a configuração dinâmica
    # Certifique-se que o método __init__ da CEAFSystem carrega o ceaf_dynamic_config
    ...
    if (datetime.now().timestamp() - last_proactive_ts) < min_proactive_interval:
        logger.info(
            f"AURA-PROACTIVE: Ação suprimida pelo intervalo (Faltam {(min_proactive_interval - (datetime.now().timestamp() - last_proactive_ts)) / 60:.1f} min)")
        return

    logger.critical(f"🚀 AURA-PROACTIVE: Agente {agent_id} disparando ação!")
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            # --- O BLOCO QUE VOCÊ PEDIU FICA AQUI ---
            if response.status_code == 200:
                logger.critical(f"✅ AURA-PROACTIVE: Mensagem enviada para o usuário {user_id}")

                # Atualiza o timestamp na memória do objeto
                agent_instance.ceaf_dynamic_config["last_proactive_message_ts"] = datetime.now().timestamp()

                # Salva no disco imediatamente para persistência
                ...
                # Reseta levemente os drives após a ação para evitar spam
                ...
                logger.error(f"❌ AURA-PROACTIVE: Falha no despacho API. Status: {response.status_code}")

    except Exception as e:
        logger.error(f"❌ AURA-PROACTIVE: Erro fatal no despacho: {e}", exc_info=True)
