...
async def investigation_activity(state_dict: Dict[str, Any], intent_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Search Memories
    memories = await ctx.memory_service.search_raw_memories(intent, top_k=15)
    ...
    # Sandbox Investigation (Optional/Costly)
    evidence = raw_text
    # Uncomment to enable E2B Sandbox usage if configured
    # if os.getenv("E2B_API_KEY"):
    #     evidence = await ctx.rlm.investigate(intent.query_vector.source_text, raw_text)

    return {"memory_context": evidence}
...
async def synthesis_activity(
    ...
    # Fetch Self Model (Identity)
    # Ideally this is cached or fetched via NCIM.
    # For now, we assume a standard self model or fetch from MBS
    ...
    response_text = await ctx.gth.translate(
        ...
        supporting_memories=[],  # Passed via context string usually, but GTH takes objects
        # We can construct dummy memories from the memory_context string if needed,
        # or update GTH to accept string context.
        # For this refactor, we'll pass empty list as memory_context string is not directly compatible
        # without parsing. In Phase 4/5 we optimize this data flow.
        user_model=None,  # Placeholder
        ...
        drives=drives,
        turn_context={"operational_advice": hormonal_data.get("hormonal_injection")},
